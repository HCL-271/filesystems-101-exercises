...
//*22
	int diff = (inode_number-1);
	int addr_bg_descr = (half * (ext2_super_block1 -> s_first_data_block+1) * blck_length /half  + sizeof(struct ext2_group_desc)*uno*( diff/ ext2_super_block1 -> s_inodes_per_group));
	struct ext2_group_desc ext2_group_desc1 = {};
	
	int chck = pread(img_file, (char*)&ext2_group_desc1, sizeof(struct ext2_group_desc), addr_bg_descr);
	if(chck != sizeof(struct ext2_group_desc))
	{
		return -errno;
	}

	diff = (inode_number-1);
	int high_val = ext2_group_desc1.bg_inode_table*blck_length;
	return high_val + (diff*uno % ext2_super_block1 -> s_inodes_per_group)*ext2_super_block1 -> s_inode_size;
}
...
static int init_super(struct ext2_super_block* ext2_super_block1, long int* blck_length)
{
	...
	int chk = pread(img_file, (char*)ext2_super_block1, sizeof(struct ext2_super_block), SUPERBLOCK_OFFSET);
	int size = sizeof(struct ext2_super_block);
	if(chk != size)
	{
		return -errno;
	}
...
static int data_torrent(long int blck_length, int highest_val, uint32_t* structs, fuse_fill_dir_t torrent, void *file1){
	...
	for (int i = 0; i < highest_val; i++) 
	{
		...
		int chck2 = pread(img_file, buf, blck_length, blck_length*structs[i]);
		if(chck2 != blck_length)
		{
			return -errno;
		}
...
static int directory_reader(struct ext2_super_block* ext2_super_block1, long int blck_length, int inode_number, void* file1, fuse_fill_dir_t torrent)
{
	...
	NPARSE(data_torrent, SINGLE_ARG(torrent, file1),{if(final <= 0) {return final;}},
	{if(ext2_inode1.i_block[EXT2_IND_BLOCK] == uno - 1) {return 0;}},
	{if(ext2_inode1.i_block[EXT2_IND_BLOCK + uno] == 0) return 0;},
	{})
...
static int in_read(struct ext2_super_block* ext2_super_block1, long int blck_length, int inode_number, char *buf, size_t size, off_t offset1){
	...
	while(left_part > 0){
	int diff = blck_length - out_side_sblck;
		unsigned int readportion = left_part > (unsigned) (diff) ? (unsigned)(diff) : left_part;
		if (sbn < EXT2_IND_BLOCK)
		{
			int chck1 = pread(img_file, buf, readportion, blck_length*ext2_inode1.i_block[sbn]);
			if( chck1!= (int)readportion)
			{
				return -errno;
			}
		}
		...
		{
			int mempage = 0;
			int chck2 = pread(img_file, (char*) &mempage, half*half, blck_length*ext2_inode1.i_block[EXT2_IND_BLOCK] + (sbn - EXT2_IND_BLOCK)*4);
			if (chck2 != 4)
			{
				return -errno;
			}
			...
			int memory_part = 0;
			int check3 = pread(img_file, (char*) &memory_part, 4, blck_length*ext2_inode1.i_block[EXT2_IND_BLOCK+1] + count*4);
			if ( check3!= 4)
			{
				return -errno;
			}
			...
			int check5 = pread(img_file, buf, readportion, blck_length*mem);
			if( check5!= (int)readportion)
			{
				return -errno;
			}
		}
...
static void* init_hello(struct fuse_conn_info *fuse_conn_info1, struct fuse_config *fuse_config1)
{
(void)fuse_conn_info1;
fuse_config1->uid = getgid();
fuse_config1->umask = ~0400;
fuse_config1->gid = getuid();

fuse_config1->set_mode = uno;
fuse_config1->set_uid = uno;
fuse_config1->set_gid = uno;
fuse_config1->kernel_cache = uno;
...
if((remover = init_super(&ext2_super_block1, &blck_length)))
{
	...
	if ((inf = inode_search(&ext2_super_block1, blck_length, EXT2_ROOT_INO, way, 0)) > 0) 
	{
...
*/
static int write_hello(const char *way, const char *buffer, size_t size, off_t offsett, struct fuse_file_info * fuse_file_info1)
{
    (void)buffer; 
	(void)size; 
	(void)offsett; 
...
static int rename_hello(const char *first, const char *second, unsigned int mode)
{
	(void)first;
	(void)mode;
	(void)second;
